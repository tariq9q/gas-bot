...
def get_current_batch_info():
    ...
    if today < BASE_START_DATE:
        return BASE_START_DATE, BASE_START_DATE + datetime.timedelta(days=CYCLE_DAYS - 1)
    
    days_passed = (today - BASE_START_DATE).days
    batch_index = days_passed // CYCLE_DAYS
    
    start_date = BASE_START_DATE + datetime.timedelta(days=batch_index * CYCLE_DAYS)
...
def get_gas_status():
    ...
    if today == start_date:
        ...
        msg += f"⏳ متبقي {days_left} أيام لتنتهي هذه الوجبة (تنتهي يوم {end_date.strftime('%Y-%m-%d')}).\n\n"
